...
from studio.geometry import resolve_position
...
class ComponentPane(Frame):
    ...
    def __init__(self, master, studio, **cnf):
        ...
        self.studio = studio
        self.config(**self.style.dark)
        self._header = Frame(self, height=30, **self.style.dark, **self.style.dark_highlight_dim)
        self._header.pack(side="top", fill="x")
        self._header.pack_propagate(0)
        Label(self._header, **self.style.dark_text_passive, text="Components").pack(side="left")
    ...
    def collect_groups(self, widget_set):
        ...
        self._pool = {}
        components = self.CLASSES.get(widget_set) + layouts.layouts
